...
export default function AgentsPage() {
  ...
  // Read URL query parameters and initialize filters
  useEffect(() => {
    if (router.isReady) {
  ...
  useEffect(() => {
    if (showAddModal || showMessageModal || showEditModal) {
  ...
  }, [showAddModal, showMessageModal, showEditModal]);
  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`/api/admin/agents/${selectedAgent.id}/message`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
  ...
  const filteredAgents = agents.filter(agent => {
    ...
    const matchesStatus = statusFilter === 'all' || 
      (statusFilter === 'online' && (agent.presenceStatus || agent.status) === 'online') ||
      (statusFilter === 'offline' && (agent.presenceStatus || agent.status) !== 'online');
        ...
        <div className="min-h-screen bg-gradient-to-br from-slate-50 via-violet-50/30 to-purple-50/30 dark:from-slate-950 dark:via-slate-900 dark:to-slate-950">
          <div className="max-w-none mx-auto space-y-6 p-6">
              ...
              <div className="p-6">
                {loading ? (
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="hidden md:grid grid-cols-12 gap-4 px-4 py-3 bg-gradient-to-r from-violet-50/50 to-purple-50/50 dark:from-violet-900/20 dark:to-purple-900/20 rounded-xl border border-violet-200/50 dark:border-violet-700/30">
                      ...
                      <div className="col-span-3 font-semibold text-sm text-slate-700 dark:text-slate-300">Agent</div>
                      <div className="col-span-2 font-semibold text-sm text-slate-700 dark:text-slate-300">Department</div>
                      <div className="col-span-1 font-semibold text-sm text-slate-700 dark:text-slate-300 text-center">Tickets</div>
                      <div className="col-span-1 font-semibold text-sm text-slate-700 dark:text-slate-300 text-center">Resolved</div>
                      <div className="col-span-1 font-semibold text-sm text-slate-700 dark:text-slate-300 text-center hidden lg:block">Response</div>
                      <div className="col-span-1 font-semibold text-sm text-slate-700 dark:text-slate-300 text-center hidden lg:block">Rating</div>
                      <div className="col-span-1 font-semibold text-sm text-slate-700 dark:text-slate-300 text-center">Status</div>
                      <div className="col-span-1 font-semibold text-sm text-slate-700 dark:text-slate-300 text-center">Actions</div>
                    </div>
                    {/* List Items */}
                    {filteredAgents.map((agent) => (
                      <React.Fragment key={agent.id}>
                        {/* Desktop List View */}
                        <div
                          className={`hidden md:grid grid-cols-12 gap-4 items-center px-4 py-3 rounded-xl border-2 transition-all relative ${
                            openMenuId === agent.id ? 'z-[100]' : 'z-auto'
                          } ${
                            selectedAgentsForAssign.includes(agent.id)
                              ? 'border-violet-500 dark:border-violet-500 bg-violet-50/90 dark:bg-violet-900/30 shadow-lg shadow-violet-500/10'
                              : 'border-slate-200/50 dark:border-slate-700/50 bg-white/90 dark:bg-slate-800/90 backdrop-blur-sm hover:border-violet-300 dark:hover:border-violet-700/50 hover:shadow-md'
                          }`}
                        >
                        <div className="col-span-1 flex items-center">
                          <label className="cursor-pointer group" onClick={(e) => e.stopPropagation()}>
                        ...
                        <div className="col-span-3 flex items-center gap-3">
                          <Avatar className="w-10 h-10 ring-2 ring-violet-100 dark:ring-violet-900/50 flex-shrink-0">
                            <AvatarFallback className="bg-gradient-to-br from-violet-500 to-purple-600 text-white text-sm font-bold">
                              {agent.name.split(' ').map(n => n[0]).join('').slice(0, 2)}
                            </AvatarFallback>
                          </Avatar>
                          <div className="min-w-0">
                            <div className="font-semibold text-slate-900 dark:text-white truncate">{agent.name}</div>
                            <div className="text-xs text-slate-600 dark:text-slate-400 truncate">{agent.email || 'No email'}</div>
                          </div>
                        </div>
                        <div className="col-span-2">
                          {agent.department ? (
                            <Badge className="bg-violet-100 dark:bg-violet-900/30 text-violet-700 dark:text-violet-300 border border-violet-200 dark:border-violet-700 text-xs">
                        ...
                        <div className="col-span-1 text-center">
                          <span className="font-semibold text-slate-900 dark:text-white">{agent.performance?.ticketsResolved || 0}</span>
                        </div>
                        ...
                        <div className="col-span-1 flex items-center justify-center gap-1">
                          ...
                          <div className="relative">
                            <button
                              ref={(el) => {
                                if (el) {
                                  menuButtonRefs.current[agent.id] = el;
                                }
                              }}
                              onClick={(e) => {
                                e.stopPropagation();
                            ...
                            {openMenuId === agent.id && typeof window !== 'undefined' && (() => {
                              const buttonRect = menuButtonRefs.current[agent.id]?.getBoundingClientRect();
                              ...
                              // Calculate space within the list container
                              const spaceBelow = containerRect ? containerRect.bottom - buttonRect.bottom : window.innerHeight - buttonRect.bottom;
                              const spaceAbove = containerRect ? buttonRect.top - containerRect.top : buttonRect.top;
                              const spaceRight = containerRect ? containerRect.right - buttonRect.right : window.innerWidth - buttonRect.right;
                              const spaceLeft = containerRect ? buttonRect.left - containerRect.left : buttonRect.left;
                              
                              // Determine position
                              const shouldShowAbove = spaceBelow < dropdownHeight && spaceAbove > spaceBelow;
                              const shouldAlignLeft = spaceRight < dropdownWidth && spaceLeft > spaceRight;
                              ...
                              if (shouldShowAbove) {
                                styleObj.bottom = `${window.innerHeight - buttonRect.top + 8}px`;
                              } else {
                                styleObj.top = `${buttonRect.bottom + 8}px`;
                              }
                              
                              // Horizontal positioning - align right edge of dropdown to right edge of button
                              if (shouldAlignLeft) {
                                styleObj.left = `${Math.max(8, buttonRect.left - dropdownWidth + buttonRect.width)}px`;
                              } else {
                                styleObj.right = `${Math.max(8, window.innerWidth - buttonRect.right)}px`;
                              }
                              ...
                              return createPortal(
                                <>
                                  ...
                                  />
                                  <div 
                                    className="fixed z-[99999] w-48 bg-white dark:bg-slate-800 rounded-xl shadow-2xl border-2 border-slate-200 dark:border-slate-700 overflow-hidden"
                                    style={styleObj}
                        ...
                        <div
                          key={`mobile-${agent.id}`}
                          className={`md:hidden p-4 rounded-lg border-2 transition-all ${
                            selectedAgentsForAssign.includes(agent.id)
                        ...
                        >
                          <div className="flex items-start gap-3 mb-3">
                            <label className="cursor-pointer group mt-1" onClick={(e) => e.stopPropagation()}>
                              <div className="relative">
                                <input
                                  type="checkbox"
                                  checked={selectedAgentsForAssign.includes(agent.id)}
                                  ...
                                  className="sr-only"
                                />
                                <div className={`w-5 h-5 rounded-md border-2 transition-all duration-200 flex items-center justify-center flex-shrink-0 ${
                                  selectedAgentsForAssign.includes(agent.id)
                                    ? 'bg-violet-600 border-violet-600 dark:bg-violet-500 dark:border-violet-500 shadow-md shadow-violet-500/30' 
                                    : 'bg-white dark:bg-slate-800 border-slate-300 dark:border-slate-600 group-hover:border-violet-400 dark:group-hover:border-violet-500 group-hover:bg-violet-50 dark:group-hover:bg-violet-900/20'
                                }`}>
                            ...
                            <Avatar className="w-12 h-12 ring-2 ring-violet-100 dark:ring-violet-900/50 flex-shrink-0">
                              <AvatarFallback className="bg-gradient-to-br from-violet-500 to-purple-600 text-white text-sm font-bold">
                                {agent.name.split(' ').map(n => n[0]).join('').slice(0, 2)}
                              </AvatarFallback>
                            </Avatar>
                            <div className="flex-1 min-w-0">
                              ...
                              <div className="text-xs text-slate-600 dark:text-slate-400 truncate mb-1">{agent.email || 'No email'}</div>
                              {agent.department && (
                                <Badge className="bg-violet-100 dark:bg-violet-900/30 text-violet-700 dark:text-violet-300 border border-violet-200 dark:border-violet-700 text-xs">
                                  {agent.department.name}
                                </Badge>
                            ...
                            <div className="relative flex-shrink-0">
                              ...
                              {openMenuId === agent.id && (
                                <div className="absolute right-0 top-10 z-50 w-48 bg-white dark:bg-slate-800 rounded-xl shadow-2xl border-2 border-slate-200 dark:border-slate-700 overflow-hidden">
                                  <button
                                    onClick={(e) => {
                                      e.stopPropagation();
                                      handleMessageAgent(agent);
                                    ...
                                    className="w-full px-4 py-3 text-left text-sm font-medium text-slate-700 dark:text-slate-300 hover:bg-violet-50 dark:hover:bg-violet-900/30 transition-colors flex items-center gap-2"
                                  >
                                    <MessageSquare className="w-4 h-4" />
                                    Send Message
                                  </button>
                          ...
                          <div className="grid grid-cols-4 gap-2 pt-3 border-t border-slate-200 dark:border-slate-700">
                            <div className="text-center">
                              <div className="text-xs text-slate-600 dark:text-slate-400 mb-1">Tickets</div>
                              <div className="font-semibold text-slate-900 dark:text-white">{agent.ticketCount || 0}</div>
                            </div>
                            <div className="text-center">
                              <div className="text-xs text-slate-600 dark:text-slate-400 mb-1">Resolved</div>
                              <div className="font-semibold text-slate-900 dark:text-white">{agent.performance?.ticketsResolved || 0}</div>
                            </div>
                            <div className="text-center">
                              <div className="text-xs text-slate-600 dark:text-slate-400 mb-1">Rating</div>
                              <div className="flex items-center justify-center gap-1">
                                <span className="font-semibold text-slate-900 dark:text-white">{agent.performance?.customerRating || '0.0'}</span>
                                <Star className="w-3 h-3 fill-amber-400 text-amber-400" />
                              </div>
                            </div>
                            <div className="text-center">
                              <div className="text-xs text-slate-600 dark:text-slate-400 mb-1">Response</div>
                              <div className="font-semibold text-slate-900 dark:text-white text-xs">{agent.performance?.avgResponseTime || 0}m</div>
                            </div>
                          </div>
                        </div>
                        {/* Mobile Compact Card View */}
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 lg:gap-6">
                    {filteredAgents.map((agent) => (
                      <div 
                        key={agent.id} 
                        className={`group p-6 rounded-2xl border-2 bg-white/90 dark:bg-slate-800/90 backdrop-blur-sm hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-1 ${
                          selectedAgentsForAssign.includes(agent.id)
                            ? 'border-violet-500 dark:border-violet-500 bg-violet-50/90 dark:bg-violet-900/30 shadow-lg shadow-violet-500/20'
                            : 'border-slate-200/50 dark:border-slate-700/50 hover:border-violet-300 dark:hover:border-violet-700/50'
                        }`}
                      >
                        {/* Header Section: Checkbox + Avatar + Info + Menu */}
                        <div className="flex items-start gap-3 mb-4">
                          ...
                          <div className="flex items-center gap-4 flex-1 min-w-0">
                            <Avatar className="w-16 h-16 ring-4 ring-violet-100 dark:ring-violet-900/50 shadow-lg group-hover:ring-violet-300 dark:group-hover:ring-violet-700 transition-all flex-shrink-0">
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              {agent.department && (
                                <div className="flex items-center gap-2 text-xs text-violet-600 dark:text-violet-400 mt-1">
                                  <Building2 className="w-3 h-3 flex-shrink-0" />
                                  <span className="truncate">{agent.department.name}</span>
                                </div>
                              )}
                            </div>
                          </div>
                          <div className="relative flex-shrink-0">
                            <button
                              ...
                              className="p-2 text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:hover:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-lg transition-all"
                            >
                              <MoreVertical className="w-5 h-5" />
                            </button>
                            {openMenuId === agent.id && (
                        ...
                        <div className="grid grid-cols-2 gap-3 mb-4">
                          <div className="p-4 bg-gradient-to-br from-blue-50 to-indigo-50 dark:from-blue-900/30 dark:to-indigo-900/30 rounded-xl border border-blue-200/50 dark:border-blue-800/50 shadow-sm">
                            <div className="flex items-center gap-2 mb-2">
                              <TicketIcon className="w-4 h-4 text-blue-600 dark:text-blue-400" />
                              <span className="text-xs font-semibold text-slate-700 dark:text-slate-300 uppercase tracking-wide">Tickets</span>
                            </div>
                            <p className="text-2xl font-bold text-slate-900 dark:text-white">{agent.ticketCount || 0}</p>
                          </div>
                          
                          <div className="p-4 bg-gradient-to-br from-emerald-50 to-green-50 dark:from-emerald-900/30 dark:to-green-900/30 rounded-xl border border-emerald-200/50 dark:border-emerald-800/50 shadow-sm">
                            <div className="flex items-center gap-2 mb-2">
                              <CheckCircle2 className="w-4 h-4 text-emerald-600 dark:text-emerald-400" />
                              <span className="text-xs font-semibold text-slate-700 dark:text-slate-300 uppercase tracking-wide">Resolved</span>
                            </div>
                            <p className="text-2xl font-bold text-slate-900 dark:text-white">{agent.performance?.ticketsResolved || 0}</p>
                          </div>
                        </div>

                        {/* Performance Metrics */}
                        <div className="space-y-3 pt-4 border-t border-slate-200/50 dark:border-slate-700/50">
                          <div className="flex items-center justify-between p-2 rounded-lg bg-slate-50/50 dark:bg-slate-900/30">
                            <div className="flex items-center gap-2">
                              <Clock className="w-4 h-4 text-violet-600 dark:text-violet-400" />
                              <span className="text-sm font-medium text-slate-700 dark:text-slate-300">Response Time</span>
                            </div>
                            <span className="text-sm font-bold text-slate-900 dark:text-white">{agent.performance?.avgResponseTime || 0}m</span>
                          </div>
                          ...
                          <div className="flex items-center justify-between p-2 rounded-lg bg-slate-50/50 dark:bg-slate-900/30">
                            ...
                            <span className="text-sm font-semibold text-slate-700 dark:text-slate-300">{formatLastActive(agent.lastActive)}</span>
                          </div>
                        </div>
                        ...
                        <div className="mt-4 pt-4 border-t border-slate-200/50 dark:border-slate-700/50 flex items-center gap-2">
                          <Button 
                            ...
                            variant="outline" 
                            size="sm"
                            className="flex-1 border-violet-200 dark:border-violet-700/50 text-violet-700 dark:text-violet-300 hover:bg-violet-50 dark:hover:bg-violet-900/30 rounded-xl transition-all"
                          >
                          ...
                          <Button 
                            ...
                            variant="outline" 
                            size="sm"
                            className="flex-1 border-slate-200 dark:border-slate-700/50 text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700/50 rounded-xl transition-all"
                          >
                            View Profile
                          </Button>
                        </div>
            ...
            {showMessageModal && selectedAgent && (
              <div 
                className="fixed inset-0 bg-black/50 dark:bg-black/70 z-50 overflow-y-auto"
                style={{
                  position: 'fixed',
                  top: 0,
                  left: 0,
                ...
                <div className="min-h-screen flex items-center justify-center p-4 py-12">
                  ...
                  <div className="flex items-center justify-between mb-6">
                    <div>
                      ...
                      <p className="text-sm text-slate-600 dark:text-slate-400 mt-1">To: {selectedAgent.name}</p>
                    </div>
                    <button
                      onClick={() => {
                        setShowMessageModal(false);
                  ...
                  <form onSubmit={handleSendMessage} className="space-y-4">
                    <div>
                      ...
                      <textarea
                        ...
                        required
                        rows={6}
                        className="w-full px-4 py-3 border-2 border-slate-300 dark:border-slate-700 dark:bg-slate-900 dark:text-white rounded-xl focus:border-violet-500 dark:focus:border-violet-500 focus:ring-2 focus:ring-violet-500/20 focus:outline-none transition-all resize-none"
                      />
                    </div>
                    ...
                    <div className="flex items-center gap-3 pt-4">
                      <Button
                        ...
                        className="flex-1 border-slate-300 dark:border-slate-700 text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700"
