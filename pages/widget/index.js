// Customer Widget - Main Entry Point
// This will be embedded on customer websites
'use client';
...
export default function CustomerWidget() {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      const savedState = localStorage.getItem('widget-open');
      if (savedState === 'true') {
        setIsOpen(true);
  ...
  const handleToggle = () => {
    ...
    if (typeof window !== 'undefined') {
      localStorage.setItem('widget-open', (!isOpen).toString());
    }
  };
