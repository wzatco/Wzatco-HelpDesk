...
/**
 * Get captcha settings including enabled placements
 * This is a server-side function
 */
export async function getCaptchaSettings() {
  try {
    ...
    const placementsValue = settingsObj['captcha_enabled_placements'];
    if (placementsValue) {
      try {
    ...
    return {
      captchaLength: parseInt(settingsObj['captcha_length'] || '6', 10),
      captchaType: settingsObj['captcha_type'] || 'alphanumeric',
      enabledPlacements: enabledPlacements
    };
  } catch (error) {
    console.error('Error fetching captcha settings:', error);
    // Return defaults on any error - always disable adminLogin
    return {
      captchaLength: 6,
