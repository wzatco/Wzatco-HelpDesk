 ...
 */
export async function calculateAgentTAT(prisma, conversationId) {
  try {
 ...
 */
export async function updateTATMetrics(prisma, conversationId) {
  try {
    const [agentTAT, firstResponse, resolutionTime] = await Promise.all([
      calculateAgentTAT(prisma, conversationId),
      calculateFirstResponseTime(prisma, conversationId),
      calculateResolutionTime(prisma, conversationId)
    ]);
