...
export async function checkTicketSLARisk(ticketId) {
  ...
  try {
    const ticket = await prismaInstance.conversation.findUnique({
      where: { id: ticketId },
      include: {
        department: {
          select: {
    ...
    if (!ticket.firstResponseAt && ticket.firstResponseTimeSeconds === null) {
      ...
      if (timeRemaining > 0 && timeRemaining <= riskThreshold) {
        ...
        const recentNotification = await prismaInstance.notification.findFirst({
          where: {
            type: 'sla_risk',
            metadata: {
              contains: `"ticketId":"${ticket.id}"`
        ...
        if (!recentNotification) {
          const notification = await notifySLARisk(prismaInstance, {
            ticketId: ticket.id,
            ticketSubject: ticket.subject || 'No subject',
            slaType: 'first_response',
            timeRemaining,
